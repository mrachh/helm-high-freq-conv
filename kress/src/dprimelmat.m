function [T,varargout] = dprimelmat(kh,src,t,spars)%N,L)
...
chi_kr = ones(size(kernel_m));
if(spars.ifsplit) 
    chi_kr = exp(-36*(kh*rr/spars.rfac/pi).^8);
end


kernel_m1 = -1 / ( 2 * pi ) * besselj(0, kh * rr).*chi_kr;
...
varargout{1} = kernel_k1;
varargout{2} = kernel_k2;
